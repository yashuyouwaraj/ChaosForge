...
export function CreateSimulationPanel() {
  ...
  const hasSelectedRun = Boolean(selectedRun.projectId) && Boolean(selectedRun.runId);
  const selectedRunStatus = selectedRun.status || "completed";

  const canPause = hasSelectedRun && selectedRunStatus === "running";
  const canResume = hasSelectedRun && selectedRunStatus === "paused";
  const canStop =
    hasSelectedRun &&
    (selectedRunStatus === "running" || selectedRunStatus === "paused");

  const handleControlAction = async (actionName, action, message) => {
    if (!hasSelectedRun) {
    ...
    try {
      ...
      setControlError(err.message || "Failed to update run control.");
    } finally {
      setActionLoading("");
    }
  };
  ...
  const handleRateUpdate = async () => {
    ...
    if (!Number.isFinite(controlRate) || controlRate <= 0) {
    ...
    try {
      ...
      setControlMessage(`RPS updated to ${controlRate}.`);
    } catch (err) {
      setControlError(err.message || "Failed to update RPS.");
    } finally {
      setActionLoading("");
    }
  };
  ...
  const handleStart = async () => {
    ...
    try {
      ...
      if (
        form.mode === "requests" &&
        (!Number.isFinite(form.totalRequests) || form.totalRequests <= 0)
      ) {
      ...
      if (
        form.mode === "duration" &&
        (!Number.isFinite(form.duration) || form.duration <= 0)
      ) {
      ...
      >
        <div>
          ...
          <div
            ...
            "
          >
            <span>Run: {selectedRun.runId || "No run selected"}</span>
        ...
        >
          <div className="grid gap-3 sm:grid-cols-3">
            <button
              type="button"
              onClick={() =>
                handleControlAction("pause", pauseRun, "Simulation paused.")
              }
            ...
            <button
              type="button"
              onClick={() =>
                handleControlAction("resume", resumeRun, "Simulation resumed.")
              }
            ...
            <button
              type="button"
              onClick={() =>
                handleControlAction("stop", stopRun, "Simulation stopped.")
              }
